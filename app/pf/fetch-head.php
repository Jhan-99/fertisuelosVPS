            ...
            //FUNCION PARA OBTTENER LOS DATOS DE UN CULTIVO EN EL SELECT
                 function obtener_siembras($conexion) {  
                  ...
                  {  
                   $output .= '
				 <option codcult="'.$row["Cultivo_siembra"].'"value="'.$row["id_siembra"].'">'.$row["Nombre_siembra"].'</option>';  
                  }  
                  return $output;  
             }  
			...
			$queryfertis = "SELECT * FROM fertilizantes";
			$result_fertis = mysqli_query($conexion, $queryfertis);
            ?>
